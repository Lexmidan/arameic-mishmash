...
cejchovani=pd.read_csv(f"{path}6_cejchovani - List 1.csv", header=0)
cejchovani['p[Pa]']=cejchovani['p[Pa]'].str.replace(',','.').astype(float)

def plot_param(ax): #not mine
    FONT = 'Times New Roman'   
    plt.ylabel('n[-]', fontname = FONT, fontweight = 'bold', fontsize = 18)
...
cejch=cejchovani[['n[-]','p[Pa]']]

cejch_fit=split_fit(cejch,fitfunc, extraplus=-4, extraminus=-4)

fig, ax=plt.subplots()

dataline=ax.scatter(cejch['n[-]'], cejch['p[Pa]'] ,color='red', s=20, marker="D")
datalines=ax.plot(cejch['n[-]'], cejch['p[Pa]'] ,color='green', alpha=0.5)
fit=ax.plot(cejch_fit['n[-]'], cejch_fit['p[Pa]'] ,color='blue',\
            label=r'$y=-(1.6\pm 0.2)x+63\pm 4$',alpha=0.7, linewidth=2)
